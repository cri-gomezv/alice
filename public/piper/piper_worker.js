...
async function init(data, phonemizeOnly = false) {
  ...
  // ONNX Runtime Web ≥1.16 requiere BigInt64Array para tensores int64.
  // Las versiones anteriores aceptaban arrays de números normales.
  const toInt64 = (arr) => BigInt64Array.from(Array.from(arr).map(BigInt));
  const feeds = {
    input:         new ort.Tensor("int64", toInt64(phonemeIds), [1, phonemeIds.length]),
    input_lengths: new ort.Tensor("int64", toInt64([phonemeIds.length])),
    scales:        new ort.Tensor("float32", [noiseScale, lengthScale, noiseW])
  };
  if (Object.keys(modelConfig.speaker_id_map).length)
    feeds.sid = new ort.Tensor("int64", toInt64([speakerId]));
  const {
    output: { data: pcm }
